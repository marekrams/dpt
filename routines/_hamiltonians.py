...
def initial_position(H, param):
    ...
    for site in psi.sweep(to='first'):
        ...
        if site >= NS:
            nl = (0, param["occLR"] * (2 * NW + NS - site) / (2 * NW))
...
def Hamiltonian_dpt_2U_mixed(NW, muL, muR, muS, dmuS, vS, U, w0=1, order='mixed0', sym='U1'):
    ...
    onsite_eng = {}
    for k in range(1, NW+1): # on-site energies
        onsite_eng[L(k)] = muL + 2 * vL * np.cos(np.pi * k / NW1)
        onsite_eng[R(k)] = muR + 2 * vR * np.cos(np.pi * k / NW1)
    onsite_eng[S(1)] = muS + dmuS
    onsite_eng[S(2)] = muS - dmuS
    ...
    if order == 'mixed0':
        ...
        raise ValueError("Hamiltonian_dpt_2U_mixed only mixed order defined.")

    s2i = {s: i for i, s in enumerate(sites)}
    i2s = {i: s for i, s in enumerate(sites)}

    cp, c, n, one = ops.cp(), ops.c(), ops.n(), ops.I()
    terms = [(v, [k], [n])  for k, v in onsite_eng.items()]

    terms.append((vS, [S(1), S(2)], [cp, c]))
    terms.append((vS, [S(2), S(1)], [cp, c]))
    for kl in range(1, NW1):
        for kr in range(1, NW1):
            amp = vLR * (2 / NW1) * np.sin(np.pi * 1 * kl / NW1) * np.sin(np.pi * 1 * kr / NW1)
            terms.append((amp, [L(kl), R(kr)], [cp, c]))
            terms.append((amp, [R(kr), L(kl)], [cp, c]))

    for k1 in range(1, NW1):
       for k2 in range(1, NW1):
            amp = U * (2 / NW1) * np.sin(np.pi * 1 * k1 / NW1) * np.sin(np.pi * 1 * k2 / NW1)
            terms.append((amp, (S(1), L(k1), L(k2)), (n, cp, c)))
            terms.append((amp, (S(1), R(k1), R(k2)), (n, cp, c)))

    H = order_generate_H(terms, s2i, N, one)
    return H, s2i, i2s


def product_state_occ(occL, occR, occS, i2s, sym='U1'):
    """ generate MPO for DPT in position basis. """
    ops = yastn.operators.SpinlessFermions(sym=sym)
    vecs = []
    occs = {'L': 0, 'R': 0, 'S': 0}
    occs_target = {'L': occL, 'R': occR, 'S': occS}
    for k in sorted(i2s.keys()):
        res = i2s[k][0]
        do = occs[res] < occs_target[res]
        occs[res] += do
        vecs.append(ops.vec_n(val=do))
    psi = mps.product_mps(vecs)
    return psi
